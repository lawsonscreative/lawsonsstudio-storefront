...
const STANDARD_COLORS = [
  { name: 'Arctic White', code: 'ACW' },
  { name: 'Charcoal', code: 'CHA' },
  { name: 'Electric Green', code: 'ELG' },
  { name: 'Electric Orange', code: 'ELO' },
  { name: 'Electric Pink', code: 'ELP' },
  { name: 'Electric Yellow', code: 'ELY' },
  { name: 'Fire Red', code: 'FIR' },
  { name: 'French Navy', code: 'FNA' },
  { name: 'Hot Pink', code: 'HPK' },
  { name: 'Jet Black', code: 'JBK' },
  { name: 'Purple', code: 'PUR' },
  { name: 'Royal Blue', code: 'ROY' },
  { name: 'Sapphire Blue', code: 'SAP' },
];
        ...
        <div className="grid gap-4">
          <div>
            ...
            <select
              value={colorCode}
              onChange={(e) => {
                const selected = STANDARD_COLORS.find(c => c.code === e.target.value);
                if (selected) {
                  setColorName(selected.name);
                  setColorCode(selected.code);
                }
              }}
              className="w-full rounded-lg border border-gray-300 px-4 py-3 text-gray-900 font-medium focus:border-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
            >
            ...
            <p className="mt-1 text-xs text-gray-500">
              Selected: {colorName ? `${colorName} (${colorCode})` : 'None'}
            </p>
          </div>
          <div>
